type ApparatusMessage = {
  type: "simulate";
  revision: number;
  apparatus: {
    photonRateKhz: number;
    modulationHz: number;
...
let kernel: WebAssembly.Exports | null = null;
...
function call(name: string, args: number[], fallback: () => number) {
  const fn = kernel?.[name] as ((...values: number[]) => number) | undefined;
  return fn ? fn(...args) : fallback();
}

self.onmessage = (event: MessageEvent<ApparatusMessage>) => {
  ...
  const pulseMv = call("pulse_mv", [a.quantumEfficiency, gain, a.terminationOhms], () =>
    (a.quantumEfficiency / 100) * gain * 1.602176634e-19 / 6e-9 * a.terminationOhms * 1000);
  ...
  const snr = call("lockin_snr", [signal, noiseMv, a.timeConstantMs / 1000, a.modulationHz, phaseRad], () =>
    signal * Math.abs(Math.cos(phaseRad)) / noiseMv * Math.sqrt(Math.max(.001, a.timeConstantMs / 1000 * a.modulationHz / 2)));
