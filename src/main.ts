import { createFieldRenderer, type FieldRenderer } from "./gpu-field.js";

type PortType = "light" | "hv" | "signal" | "reference" | "return";
type Param = { label: string; min: number; max: number; step: number; value: number; unit: string };
type BodyDefinition = {
  label: string; short: string; family: string; color: string; description: string;
  ports: { id: string; label: string; type: PortType; side: "left" | "right" | "top" | "bottom" }[];
  params: Record<string, Param>; dive?: string;
};
type Body = { id: string; kind: string; x: number; y: number; parameters: Record<string, number> };
type Tether = { id: string; from: { bodyId: string; portId: string }; to: { bodyId: string; portId: string }; lengthM: number; type: PortType };
type Capsule = {
  schema: number; title: string; seed: number; logicalTime: number; event: number;
  view: string; lens: string; bodies: Body[]; tethers: Tether[]; selectedBodyId: string | null;
};
type Metrics = {
  gain: number; pulseMv: number; rawPulseMv: number; snr: number; settlingMs: number;
  detectedRate: number; chargePc: number; probeLoading: number; noiseMv: number;
};
...
let hover = { bodyId: null as string | null, portId: null as string | null };
let gesture: null | {
  kind: "body" | "pan" | "wire";
...
function cloneState(): Capsule {
  return structuredClone(state);
}

function mutate(label: string, operation: () => void, record = true) {
  if (record) history.push(cloneState());
  operation();
...
function addBody(kind: string, x: number, y: number, record = true) {
...
function connect(fromBodyId: string, fromPortId: string, toBodyId: string, toPortId: string, record = true) {
  if (fromBodyId === toBodyId && fromPortId === toPortId) return false;
  ...
  if (state.tethers.some((t) =>
    (t.from.bodyId === fromBodyId && t.from.portId === fromPortId && t.to.bodyId === toBodyId && t.to.portId === toPortId) ||
    (t.to.bodyId === fromBodyId && t.to.portId === fromPortId && t.from.bodyId === toBodyId && t.from.portId === toPortId))) return false;
...
function clearLab(record = true) {
  mutate("Cleared apparatus", () => {
    state.bodies = []; state.tethers = []; state.selectedBodyId = null; state.event = 0;
...
function getBody(id: string) { return state.bodies.find((body) => body.id === id); }
function getPort(body: Body, id: string) { return DEFINITIONS[body.kind].ports.find((port) => port.id === id); }

function portPosition(body: Body, portId: string) {
  ...
  if (port.side === "left" || port.side === "right") offset = { x: offset.x, y: spread };
  else offset = { x: spread, y: offset.y };
...
function draw() {
  ...
  if (gesture?.kind === "wire" && gesture.from) {
    ...
    if (pointer) drawWire(start.x, start.y, pointer.x, pointer.y, getPort(body, gesture.from.portId)!.type, true);
  }
...
function drawWire(ax: number, ay: number, bx: number, by: number, type: PortType, loose: boolean) {
  const dx = bx - ax;
  ...
  if (type === "return") ctx.setLineDash([4, 5]); else ctx.setLineDash([]);
  ctx.stroke();
  ctx.setLineDash([]);
  if (state.event > 0 && (type === "light" || type === "signal" || type === "reference")) {
    ...
    ctx.beginPath(); ctx.arc(point.x, point.y, type === "light" ? 3.5 : 2.5, 0, Math.PI * 2);
    ctx.fillStyle = PORT_COLORS[type]; ctx.shadowBlur = 12; ctx.shadowColor = PORT_COLORS[type]; ctx.fill(); ctx.shadowBlur = 0;
  }
}
...
function drawBody(body: Body) {
  ...
  if (selected) {
    ctx.strokeStyle = colorWithAlpha(def.color, .7); ctx.lineWidth = 1;
  ...
  ctx.fillStyle = gradient; roundRect(ctx, -w/2, -h/2, w, h, 6); ctx.fill();
  ctx.strokeStyle = selected ? def.color : "rgba(218,233,228,.18)"; ctx.lineWidth = 1;
  roundRect(ctx, -w/2, -h/2, w, h, 6); ctx.stroke();
  ctx.beginPath(); ctx.arc(-w/2 + 22*camera.scale, 0, 13*camera.scale, 0, Math.PI*2);
  ctx.strokeStyle = colorWithAlpha(def.color, .55); ctx.stroke();
  ctx.fillStyle = def.color; ctx.font = `${Math.max(8, 10*camera.scale)}px 'DM Mono'`;
  ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(def.short, -w/2 + 22*camera.scale, 0);
  ctx.textAlign = "left";
  ctx.fillStyle = "#e9efec"; ctx.font = `500 ${Math.max(8, 10*camera.scale)}px Inter`;
  ctx.fillText(def.label, -w/2 + 44*camera.scale, -6*camera.scale);
  ctx.fillStyle = "rgba(197,208,204,.48)"; ctx.font = `${Math.max(6, 7*camera.scale)}px 'DM Mono'`;
  ...
  for (const port of def.ports) {
    ...
    ctx.beginPath(); ctx.arc(pos.x, pos.y, isHover ? 7 : 5, 0, Math.PI * 2);
    ctx.fillStyle = "#0a0d0e"; ctx.fill();
    ctx.strokeStyle = PORT_COLORS[port.type]; ctx.lineWidth = isHover ? 2 : 1; ctx.stroke();
    if (isHover || selected) {
      ctx.fillStyle = colorWithAlpha(PORT_COLORS[port.type], .7); ctx.font = "7px 'DM Mono'"; ctx.textAlign = "center";
...
function drawEvent() {
  ...
  ctx.beginPath(); ctx.arc(pos.x, pos.y, ring * camera.scale, 0, Math.PI * 2);
  ctx.strokeStyle = `rgba(199,255,93,${.18 - ((ring-48)/22)*.16})`; ctx.stroke();
}

function drawTimeView(width: number, height: number) {
  const panelH = 150, y0 = height - 210;
  ctx.fillStyle = "rgba(7,9,10,.86)"; ctx.fillRect(14, y0, width - 28, panelH);
  ctx.strokeStyle = "rgba(210,230,225,.12)"; ctx.strokeRect(14, y0, width - 28, panelH);
  ctx.font = "8px 'DM Mono'"; ctx.fillStyle = "rgba(220,230,226,.45)";
  ...
  ctx.strokeStyle = "#c7ff5d"; ctx.lineWidth = 1.5; ctx.stroke();
  ctx.strokeStyle = "rgba(182,156,255,.45)"; ctx.setLineDash([4,4]);
  ctx.beginPath(); ctx.moveTo(24, y0+75-pulse*1.12); ctx.lineTo(width-24, y0+75-pulse*1.12); ctx.stroke(); ctx.setLineDash([]);
}

function drawPhaseView(width: number, height: number) {
  const cx = width / 2, cy = height / 2, r = Math.min(width, height) * .25;
  ctx.fillStyle = "rgba(7,9,10,.74)"; ctx.beginPath(); ctx.arc(cx, cy, r + 26, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = "rgba(210,230,225,.13)";
  for (const f of [.33,.66,1]) { ctx.beginPath(); ctx.arc(cx,cy,r*f,0,Math.PI*2);ctx.stroke(); }
  ...
  for (let i=0;i<42;i++) {
    ...
    ctx.fillStyle = "rgba(105,231,220,.24)";ctx.beginPath();ctx.arc(cx+Math.cos(a)*rr,cy+Math.sin(a)*rr,2,0,Math.PI*2);ctx.fill();
  }
  ...
  ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx+Math.cos(phase)*strength,cy-Math.sin(phase)*strength);
  ctx.strokeStyle="#c7ff5d";ctx.lineWidth=3;ctx.stroke();
  ctx.beginPath();ctx.arc(cx+Math.cos(phase)*strength,cy-Math.sin(phase)*strength,5,0,Math.PI*2);ctx.fillStyle="#c7ff5d";ctx.fill();
  ctx.font="8px 'DM Mono'";ctx.fillStyle="rgba(220,230,226,.55)";ctx.fillText("IN PHASE",cx+r+16,cy+3);ctx.fillText("QUADRATURE",cx+7,cy-r-17);
}

function drawProbabilityView(width: number, height: number) {
  const x0 = width*.18, y0 = height*.72, w=width*.64, h=height*.44;
  ctx.fillStyle="rgba(7,9,10,.82)";ctx.fillRect(x0-24,y0-h-25,w+48,h+50);
  ctx.strokeStyle="rgba(210,230,225,.13)";ctx.strokeRect(x0-24,y0-h-25,w+48,h+50);
  const mean = Math.log10(Math.max(10,metrics?.gain||1000));
  for(let i=0;i<48;i++){
    const z=(i/47*6-3); const density=Math.exp(-.5*Math.pow(z-(mean-5)*.45,2));
  ...
  ctx.font="8px 'DM Mono'";ctx.fillStyle="rgba(220,230,226,.52)";
  ctx.fillText("10³",x0,y0+16);ctx.fillText("10⁴",x0+w*.33,y0+16);ctx.fillText("10⁵",x0+w*.66,y0+16);ctx.fillText("10⁶ e⁻",x0+w-25,y0+16);
...
canvas.addEventListener("pointerdown", (event) => {
  ...
  if (pointers.size === 2) {
    const [a,b]=[...pointers.values()]; pinchStart={distance:Math.hypot(a.x-b.x,a.y-b.y),scale:camera.scale}; return;
  }
...
canvas.addEventListener("pointermove",(event)=>{
  const point=localPoint(event);pointers.set(event.pointerId,point);
  if(pinchStart&&pointers.size>=2){const[a,b]=[...pointers.values()];camera.scale=clamp(pinchStart.scale*Math.hypot(a.x-b.x,a.y-b.y)/pinchStart.distance,.45,2.5);return;}
...
canvas.addEventListener("dblclick",(event)=>{const p=localPoint(event),hit=hitTest(p.x,p.y);if(hit?.body)openDive(hit.body);});
canvas.addEventListener("wheel",(event)=>{
  event.preventDefault();const p=localPoint(event);const before=screenToWorld(p.x,p.y);camera.scale=clamp(camera.scale*Math.exp(-event.deltaY*.0012),.45,2.5);
  const after=screenToWorld(p.x,p.y);camera.x+=after.x-before.x;camera.y+=after.y-before.y;
},{passive:false});

function localPoint(event: MouseEvent | PointerEvent | WheelEvent) {
...
function updateMetrics(){
  ...
  const card=$("#challenge");card.classList.toggle("solved",solved);
...
function renderInspector(body:Body){
  const def=DEFINITIONS[body.kind];$("#inspector-title").textContent=def.label;
  ...
  const list=$("#parameter-list");list.innerHTML="";
  ...
  const diveButton=$("#dive-button") as HTMLButtonElement;diveButton.disabled=!def.dive;diveButton.textContent=def.dive?"Enter body":"No authored interior";
}
function formatParameter(value:number,unit:string){return `${Number.isInteger(value)?value:value.toFixed(1)}${unit?` ${unit}`:""}`;}
function escapeHtml(text:string){const d=document.createElement("div");d.textContent=text;return d.innerHTML;}

function openDive(body:Body){
  const def=DEFINITIONS[body.kind];if(!def.dive){showToast("This body has no authored interior yet");return;}
  state.selectedBodyId=body.id;$("#dive-title").textContent=`Inside ${def.label}`;$("#dive-copy").textContent=def.dive;
  const dialog=$("#dive-dialog") as HTMLDialogElement;dialog.showModal();drawDive(body);
}
function drawDive(body:Body){
  const dive=$("#dive-canvas") as HTMLCanvasElement;const dctx=dive.getContext("2d")!;const w=dive.width,h=dive.height;
  dctx.clearRect(0,0,w,h);dctx.fillStyle="#07090a";dctx.fillRect(0,0,w,h);
  if(body.kind==="photomultiplier"){
    const gradient=dctx.createLinearGradient(0,0,w,0);gradient.addColorStop(0,"rgba(105,231,220,.12)");gradient.addColorStop(.6,"rgba(199,255,93,.07)");gradient.addColorStop(1,"transparent");dctx.fillStyle=gradient;dctx.fillRect(0,0,w,h);
    dctx.strokeStyle="rgba(220,235,230,.25)";dctx.lineWidth=2;dctx.beginPath();dctx.moveTo(80,110);dctx.bezierCurveTo(10,310,40,500,185,525);dctx.lineTo(1050,525);dctx.bezierCurveTo(1160,510,1165,150,1050,110);dctx.closePath();dctx.stroke();
    dctx.fillStyle="rgba(105,231,220,.18)";dctx.fillRect(105,125,18,360);dctx.fillStyle="#69e7dc";dctx.font="13px 'DM Mono'";dctx.fillText("PHOTOCATHODE",55,90);
    const stages=Math.round(body.parameters.stages);const startX=210,spacing=720/Math.max(1,stages-1);
    for(let i=0;i<stages;i++){const x=startX+i*spacing,y=210+(i%2)*165;dctx.save();dctx.translate(x,y);dctx.rotate(i%2?.32:-.32);dctx.fillStyle="rgba(199,255,93,.14)";dctx.strokeStyle="rgba(199,255,93,.55)";dctx.fillRect(-22,-42,44,84);dctx.strokeRect(-22,-42,44,84);dctx.restore();dctx.fillStyle="rgba(220,235,230,.45)";dctx.font="10px 'DM Mono'";dctx.fillText(`D${i+1}`,x-10,y+(i%2?-62:67));}
    const event=Math.max(1,state.event);let px=120,py=305;
    for(let i=0;i<=stages;i++){const nx=i<stages?startX+i*spacing:1010,ny=i<stages?210+(i%2)*165:305;const count=Math.min(22,1+Math.floor(i*i*.22));for(let n=0;n<count;n++){const t=((performance.now()/1100+n*.077+event*.13)%1);const x=px+(nx-px)*t,y=py+(ny-py)*t+seededNoise(i*90+n)*10;dctx.beginPath();dctx.arc(x,y,2.2,0,Math.PI*2);dctx.fillStyle="#c7ff5d";dctx.fill();}px=nx;py=ny;}
    dctx.fillStyle="#c7ff5d";dctx.font="11px 'DM Mono'";dctx.fillText(`${metrics?.gain.toExponential(2)||"—"} ELECTRONS / DETECTED PHOTON`,820,82);
  }else if(body.kind==="lock-in"){
    const cx=w/2,cy=h/2,r=210;dctx.strokeStyle="rgba(220,235,230,.13)";
    for(const f of [.25,.5,.75,1]){dctx.beginPath();dctx.arc(cx,cy,r*f,0,Math.PI*2);dctx.stroke();}
    dctx.beginPath();dctx.moveTo(cx-r-40,cy);dctx.lineTo(cx+r+40,cy);dctx.moveTo(cx,cy-r-40);dctx.lineTo(cx,cy+r+40);dctx.stroke();
    const angle=-body.parameters.phaseDeg*Math.PI/180;dctx.strokeStyle="#c7ff5d";dctx.lineWidth=5;dctx.beginPath();dctx.moveTo(cx,cy);dctx.lineTo(cx+Math.cos(angle)*r*.82,cy+Math.sin(angle)*r*.82);dctx.stroke();
    dctx.fillStyle="#c7ff5d";dctx.font="12px 'DM Mono'";dctx.fillText("REFERENCE-COHERENT COMPONENT",cx+Math.cos(angle)*r*.5,cy+Math.sin(angle)*r*.5-14);
    dctx.fillStyle="rgba(220,235,230,.45)";dctx.fillText("I",cx+r+30,cy+4);dctx.fillText("Q",cx+5,cy-r-26);
  }else{
    dctx.fillStyle="rgba(220,235,230,.5)";dctx.font="18px 'DM Mono'";dctx.textAlign="center";dctx.fillText(definitionInterior(body.kind),w/2,h/2);
  }
}
function definitionInterior(kind:string){return kind==="probe"?"REAL INPUT IMPEDANCE · NOT AN IDEAL OBSERVER":kind==="photon-source"?"VERSIONED OPTICAL STIMULUS BOUNDARY":"AUTHORED PHYSICAL INTERIOR";}
...
function restore(){
  if(!location.hash.startsWith("#es1."))return false;
  try{
