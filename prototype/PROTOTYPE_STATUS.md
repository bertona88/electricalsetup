...
For **ElectricalSetup**, https://electricalsetup.com/ was verified against that release on 2026-07-26; current availability must be checked live. The local preview route is `/?setup=electrical`.

The snapshot is intentionally not split into a domain-specific architecture: that production release serves one common static runtime and resolves the active setup from the hostname. The root [AGENTS.md](../AGENTS.md) is authoritative: future ElectricalSetup work starts from a new design outside this snapshot.
